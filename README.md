...
- **`cargo-fuzz` harness**, one must-not-panic target per parsed structure (`fuzz_ese_open`, `fuzz_catalog`, `fuzz_page`, `fuzz_record`, `fuzz_integrity`, `fuzz_carver`). The first smoke pass found and fixed a real page-number overflow. See [validation](docs/validation.md).
...
`ese-core` is extracted from and shared with [`srum-forensic`](https://github.com/SecurityRonin/srum-forensic); [`browser-forensic`](https://github.com/SecurityRonin/browser-forensic) and other consumers reuse it. Real-database validation currently rests on a third-party SRUDB.dat corpus exercised in the sibling repos; a `libesedb`/`esedbexport` differential oracle is the next validation step ([details](docs/validation.md)).
